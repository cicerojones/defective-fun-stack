...
* model of testing suite for original Udacity module
** commented functions
#+BEGIN_SRC python :session *Python* :results output
import psycopg2
def deleteMatches():
...
: 
: hello
* last steps and current planning
** steps
1. check to see if Python functions from the original test suite are
   working

   - this may involve knowing what code blocks need to be evaluated,
     in what order.

   - for example, should [[file:merge-final-project-and-extra-credit.org::*all%20Python%20definitions%20in%20one%20block][all Python definitions in one block]] be
     evaluated from the get-go?

     - probably not, since they contain definitions of functions that
       are using columns that we may not be wanting.

   - better to go through chunks of acceptable code at a time
