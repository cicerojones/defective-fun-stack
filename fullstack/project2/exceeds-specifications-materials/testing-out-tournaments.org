...
#+END_SRC
* [#C] (broken up) three rounds plus one round second tournament
  :PROPERTIES:
  :ID:       83580BFC-C987-43C7-B636-49264D564F66
  :END:
** see 'run tournaments' below for most concise outline
** code blocks succeed in sequence (but not all at once?)
...
*** uncommented version of first round code (only one round?!)
...
#+RESULTS:
*** stage following initial setup and results for first tournament
...
#+END_SRC
* most broken up
** templates for saving in registers
...
#+END_SRC
** 12 individual blocks that work as two tournaments
   :PROPERTIES:
   :ID:       9055D4F6-669D-4341-AF67-E79FAD5E692E
   :END:
*** register players for 'tennis;' register 1st round matches
#+BEGIN_SRC python :session *Python* :results output :tangle yes
...
*** register score for first round; log results/crunch numbers
#+BEGIN_SRC python :session *Python* :results output
...
*** register scores for matches (created by swissPairings); crunch data
#+BEGIN_SRC python :session *Python* :results output

registerScores("tourney_practice", "score_results", 5, 1, 0)
# registerScores("tourney_practice", "score_results", 5, 0, 1)
registerScores("tourney_practice", "score_results", 6, 0, 1)
registerScores("tourney_practice", "score_results", 7, 1, 0)
registerScores("tourney_practice", "score_results", 8, 0, 1)
...
*** register scores to end tennis tournament; crunch numbers; winner implicit
#+BEGIN_SRC python :session *Python* :results output
registerScores("tourney_practice", "score_results", 9, 1, 0)
registerScores("tourney_practice", "score_results", 10, 0, 1)
registerScores("tourney_practice", "score_results", 11, 1, 0)
...
*** first round soccer scores/crunch numbers
#+BEGIN_SRC python :session *Python* :results output
...
*** swisspairing for round two of soccer
#+BEGIN_SRC python :session *Python* :results output
naive_swissPairings(2, "soccer")
#+END_SRC
*** register scores for round two of score/crunch numbers
#+BEGIN_SRC python :session *Python* :results output
## 
registerScores("tourney_practice", "score_results", 17, 1, 1)
registerScores("tourney_practice", "score_results", 18, 0, 1)
...
set_all_OMW('tourney_practice')
#+END_SRC
*** final swissPairings for 'soccer'
#+BEGIN_SRC python :session *Python* :results output
naive_swissPairings(3, "soccer")
#+END_SRC
*** register scores/crunch numbers for end of soccer; winner implicit
#+BEGIN_SRC python :session *Python* :results output
registerScores("tourney_practice", "score_results", 21, 1, 0)
registerScores("tourney_practice", "score_results", 22, 0, 1)
registerScores("tourney_practice", "score_results", 23, 1, 0)
...
set_all_OMW('tourney_practice')
#+END_SRC
** determine winner of 'tennis'--note, uses org-babel "VALUE" 
#+BEGIN_SRC python :session *Python* :results value
determine_winner("tourney_practice", "player_tables", "tennis")
#+END_SRC
...
* [#A] run tournaments (in two babel pieces--will work as one .py module)
** Players, Matches, Scores, for 3 rounds; new Players (big chunk won't work UNLESS!)
will take a long time?
- register players
- register who the players will face in a given round of a given tournament
- register scores for the given match id (uses implicit home/away knowledge)
- call functions to log results/crunch numbers
- set up next round
  - naive swissPairings encapsulates step of registering who will face
    whom in the next round, which number is given as an argument
- since matches have already been created, now just repeat parts of
  above:
  - register scores for new round
  - log results/crunch numbers
  - set up next round (round 3, in this
- repeat, this previous step (1. new scores 2. crunch numbers 3. next
  round) until a winner is determined based on characteristics of the
  particular tournament (no. of players, ties allowed etc.)
- start new tournament by registering new players
  - uses same database name ("tourney practice" in this case)
  - uses same tables in that db (primarily, "playerz", "match_participants",
    "score_results")
...
** repeat above for NEW TOURNAMENT (allow ties!)
#+BEGIN_SRC python :session *Python* :results output :tangle yes
new_registerPlayer("tourney_practice", "playerz", 'allen', "soccer")
new_registerPlayer("tourney_practice", "playerz", 'beverly', "soccer")
new_registerPlayer("tourney_practice", "playerz", 'cleanth', "soccer")
...
* [#B] run the sql setup (drop and create tables and functions)
  :PROPERTIES:
  :ID:       E646931C-37B1-496F-B545-8CE95CF75E02
  :END:
- Drops tables (with cascades) and creates again
- creates views
- finally, creates many functionsn
...
|        32 | quail           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        17 | aleph           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        18 | betel           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        19 | colon           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        20 | dippy           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        21 | eeker           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        22 | fubar           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        23 | gonad           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        24 | hilly           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        25 | isomr           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        26 | joker           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        27 | lossy           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        28 | mamie           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        29 | nadir           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        30 | oopsy           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        31 | pappi           |         mahjong |    0 |      0 |     0 |      0 |   0 |
|        16 | homers          |          soccer |    2 |      0 |     1 |      7 |  19 |
|         9 | azetcs          |          soccer |    2 |      0 |     1 |      7 |  10 |
|        12 | devils          |          soccer |    2 |      1 |     0 |      6 |  13 |
|        15 | gomers          |          soccer |    2 |      1 |     0 |      6 |  10 |
|        13 | ejects          |          soccer |    1 |      2 |     0 |      3 |  16 |
|        14 | freaks          |          soccer |    1 |      2 |     0 |      3 |  12 |
|        11 | choirs          |          soccer |    1 |      2 |     0 |      3 |   9 |
|        10 | beards          |          soccer |    0 |      3 |     0 |      0 |  16 |
|         1 | albert          |          tennis |    3 |      0 |     0 |      9 |  15 |
|         8 | harold          |          tennis |    2 |      1 |     0 |      6 |  15 |
|         2 | beverly         |          tennis |    2 |      1 |     0 |      6 |  12 |
|         4 | devon           |          tennis |    2 |      1 |     0 |      6 |  12 |
|         5 | eldridge        |          tennis |    1 |      2 |     0 |      3 |  18 |
|         6 | fatool          |          tennis |    1 |      2 |     0 |      3 |  12 |
|         3 | cleanth         |          tennis |    1 |      2 |     0 |      3 |   9 |
|         7 | g-money         |          tennis |    0 |      3 |     0 |      0 |  15 |

#+BEGIN_SRC sql :engine postgresql :database tourney_practice

select player_id from player_tables ORDER BY tournament_name, points DESC, omw DESC limit 1;
#+END_SRC
...
#+BEGIN_SRC sql :engine postgresql :database tourney_practice
select player_id from player_tables WHERE tournament_name = 'tennis' ORDER BY tournament_name, points DESC, omw DESC limit 1
#+END_SRC
...
* determine winner of given tournament from python
** sql statement for 'tennis'
#+BEGIN_SRC sql :engine postgresql :database tourney_practice
select player_id from player_tables WHERE tournament_name = 'tennis' ORDER BY tournament_name, points DESC, omw DESC limit 1
#+END_SRC
...
def new_playerStandings_alt(dbname, table_name, tournament_name):
    ...
    DB.close()
    return rows
#   why does the following not fly?
